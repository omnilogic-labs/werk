...
`test-full`'s failures are below. `daemon.test.ts` is in the failing set
rather than the no-verdict set now: teardown through the protocol closed the
file that used to panic, so nothing is hidden behind it any more.
