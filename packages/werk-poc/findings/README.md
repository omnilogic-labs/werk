...
[library-readiness.md](./library-readiness.md), the assessment and proposed
proof before extracting an internal session library;
...
| RSS grows without bound over the 24-hour soak → the daemon cannot be long-lived in Bun                       | **Not hit at 30 minutes; 24 hours not taken.** Twenty sessions held for 30 minutes: RSS fills to 116 MiB in the first minute (the noisy sessions' scrollback) and then oscillates between 116 and 120 MiB on a sawtooth about four megabytes deep; the second-half slope of +4.4 MiB/h is smaller than that sawtooth, so half an hour does not separate a trend from the cycle. Event-loop lag away from a snapshot pass has a median under 0.4 ms and a worst p99 of 3.4 ms, and attach latency at the end is no worse than at the start. The 24-hour run the proposal names is not taken; the runner and its exact command are ready. | [m6](./m6.md#the-soak)                   |
