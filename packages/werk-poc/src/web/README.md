...
| File                              | What it is                                                                                                                                                                                     |
| --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `server.ts`                       | `Bun.serve` on 127.0.0.1: the one-time token → cookie, the session list, the terminal page, `/wasm`, `/beamterm.wasm`, `/app.js`, and one WebSocket per open terminal (a snapshot-mode attach) |
...
| `client/renderer.ts`              | The renderer seam — `Renderer`, `RendererHost`, `RendererSelection`, the shared paint-stats and palette helpers — and the minimal canvas renderer behind it                                    |
...
| `client/renderer-wterm.ts`        | `@wterm/dom`'s DOM renderer driven through a `TerminalCore` adapter over `Frame`; `?renderer=wterm` selects it. Real DOM rows, so selection is the browser's own                               |
| `client/renderer-beamterm.ts`     | `@beamterm/renderer`'s WebGL2 renderer fed cell by cell from `Frame`; `?renderer=beamterm` selects it. Its 1.4 MB WASM comes from `/beamterm.wasm`, fetched only when chosen                   |
...
Four renderers sit behind one interface and are fed the same frames, so a
session can be measured through each in turn: `?renderer=` takes `minimal`
(the default), `ghostty-web`, `wterm` or `beamterm`.
