...
The accompanying `LICENSE` is the upstream MIT licence. The bytes are identical
to the research pin; the library adapter is independently implemented against
the artefact's self-describing C ABI. An asset upgrade requires a new engine
build identity and snapshot compatibility validation.
