...
/** The budget one probe command gets. Long enough for a handshake, short
 *  enough that a machine that is asleep does not hold anything up. */
 ...
 * it is the same string `probeFacts` reports as the `workspace root` check, so
...
export async function probeFacts(
  sshHost: string,
  runner: RemoteRunner,
...
export function sshProbe(sshHost: string, runner: RemoteRunner): HostProbe {
  return {
    async run(
      ...
      // The seam speaks argv so a caller never has to think about quoting; ssh
      // hands the far side one string for its shell, so the quoting happens
      // here, once, with the seam's own quoter.
      const script = command.map(shellQuote).join(" ");
      const outcome = await runner.run(
        sshExecArgv(sshHost, script, { login: options.login === true }),
