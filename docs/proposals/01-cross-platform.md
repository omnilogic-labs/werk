...
| The two Windows matrix lanes once they are gated                          | `matrix.yml`                                                               | [33713887366](https://github.com/omnilogic-labs/werk/actions/runs/33713887366)                                                                                                                                                                 |
...
ls` against a live daemon at 79–82 ms (runs 33712817886, 33712812822).
   ...
   Those four are the difference, and they are recorded on arm64 rather than
   gated: there is no `bun:ffi` there and libghostty-vt ships no prebuild
   for the target, so the ffi engine cannot load and every suite that counts
   it is red for a reason nothing in this step changes. Three more are
   recorded on both runners. `m0`: three of seven PTY probes pass, and which
   three moves between runs — `03-sigwinch` and `06-raw-mode` swap places on
   x64 between runs 33703355321 and 33712817886. `m3`: every snapshot it
   encodes decodes identically, and then the process does not exit, on some
   runs on either runner. `test-full`: the ConPTY costs §3 records. Gating
   any of those would gate on something already understood, which is worse
   than not gating it.
